...
HitResult hitBVHCore(const Ray& ray, Triangle* triangles, const Matrix& vertex_coordinates, const Matrix& AABB, BVHNode* node) {
    ...
    if (node->left_child) {
        ...
        t_left = hitAABB(ray, AA, BB);
    }
    if (node->right_child) {
        Vector3 AA(AABB[node->right_child->aa], 1);
        Vector3 BB(AABB[node->right_child->bb], 1);
        t_right = hitAABB(ray, AA, BB);
    }
    HitResult res_left, res_right;
    if (t_left > 0) res_left = hitBVHCore(ray, triangles, vertex_coordinates, AABB, node->left_child);
    if (t_right > 0) res_right = hitBVHCore(ray, triangles, vertex_coordinates, AABB, node->right_child);
