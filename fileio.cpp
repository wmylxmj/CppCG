...
void getpos(const std::string& str, float* const arr) {
    int pos = 0, n = 0, i = 0;
...
unsigned int get_triangles(const std::string& str, Triangle* const triangles) {
    ...
    for (char buffer : str) {
        if (buffer >= 48 && buffer <= 57) {
            ...
            if (n > 0) {
                set(triangles, inner, outer, (unsigned int) str2num<int>(str.substr(pos, n)));
            }
            pos += n + 1;
            n = 0;
            inner++;
...
void OBJFile::read(const std::string& fp) {
    ...
    this->vertex_coords.rebuild(nv, 3);
    this->texture_coords.rebuild(nvt, 2);
    this->normal_coords.rebuild(nvn, 3);
    this->triangles = new Triangle[ntris];
    while (std::getline(f, line)) {
        if (startswith(line, "v ")) {
            getpos<3, float>(line, this->vertex_coords[iv++]);
        }
        else if (startswith(line, "vt")) {
            getpos<2, float>(line, this->texture_coords[ivt++]);
        }
        else if (startswith(line, "vn")) {
            getpos<3, float>(line, this->normal_coords[ivn++]);
        }
        else if (startswith(line, "f ")) {
