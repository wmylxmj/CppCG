...
int main() {
    ...
    // transform
    float xmean = cblas_ssum((blasint) vertex_coordinates.shape(0), vertex_coordinates[0] + 0,
                             (blasint) vertex_coordinates.shape(1)) / (float) vertex_coordinates.shape(0);
    float ymean = cblas_ssum((blasint) vertex_coordinates.shape(0), vertex_coordinates[0] + 1,
                             (blasint) vertex_coordinates.shape(1)) / (float) vertex_coordinates.shape(0);
    float zmin = FLT_MAX;
    for (size_t i = 0; i < vertex_coordinates.shape(0); i++)
        zmin = vertex_coordinates[i][2] < zmin ? vertex_coordinates[i][2] : zmin;
    ...
    // light
    xmean = cblas_ssum((blasint) vertex_coordinates.shape(0), vertex_coordinates[0] + 0,
                       (blasint) vertex_coordinates.shape(1)) / (float) vertex_coordinates.shape(0);
    float zmean = cblas_ssum((blasint) vertex_coordinates.shape(0), vertex_coordinates[0] + 2,
                             (blasint) vertex_coordinates.shape(1)) / (float) vertex_coordinates.shape(0);
